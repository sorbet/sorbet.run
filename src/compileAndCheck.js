...
const sorbetModuleCompile = fetch('sorbet-wasm.wasm')
...
const runCPP = (flushOut, contents) => (Module) => {
...
export const typecheck = (flushOut, flushErr) => (contents) => {
  setTimeout(() => {
    compile(flushOut, flushErr).then(runCPP(flushOut, contents));
  }, 1);
};
